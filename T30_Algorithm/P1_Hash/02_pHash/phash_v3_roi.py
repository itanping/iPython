...
# ---------------------------------------------------------------------------------------------------------------------
# 多图测试：DCT变换后，只提取DCT系数左上角8x8像素：img_dct[0:8, 0:8]
# ---------------------------------------------------------------------------------------------------------------------
...
def get_hash(img_path):
    ...
    # 缩放DCT：只提取了DCT系数的左上角8x8块（即ROI，感兴趣区域）
    ...
    img_hash_str = ""
    for i in range(8):
        img_hash_str += ''.join(map(lambda i: '0' if i < img_avg else '1', dct_roi[i]))
    print(f"图像的二进制哈希值={img_hash_str}")
